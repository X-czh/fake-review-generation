...
def train(input_tensor, input_lengths, target_tensor, target_lengths, 
    ...
    for t in range(max_target_length):
        decoder_output, decoder_hidden = decoder.forward_step(decoder_input, decoder_hidden, noise)
    ...
    with torch.no_grad():
        ...
        for di in range(max_length):
            decoder_output, decoder_hidden = decoder.forward_step(
                decoder_input, decoder_hidden, noise)
            topv, topi = decoder_output.data.topk(1)
            ni = topi.item()
            if ni == EOS_token:
...
if __name__ == '__main__':
    ...
    print("noise-size: {}".format(args.noise_size))
    ...
    decoder = DecoderRNN(args.hidden_size * 2, args.vocab_size, args.noise_size, args.n_layers, args.dropout, args.cuda)
    if args.cuda:
